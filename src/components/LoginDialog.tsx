...
export const LoginDialog = ({ defaultUsername = '', open, handleClose, onLoggedIn }: LoginDialogProps) => {
  ...
  const loginForm = (
    <form
      ...
      <DialogTitle>{challengeName === 'NEW_PASSWORD_REQUIRED' ? 'Change Password' : 'Authentication'}</DialogTitle>
      <DialogContent>
        <TextField
          autoFocus
          id="id"
  ...
  const changePasswordForm = (
    <form
      ...
      <DialogActions>
        ...
        <Button type="submit" disabled={isBusy} variant="contained" color="info" onClick={handleChangePassword}>
          SUBMIT
        </Button>
      </DialogActions>
    </form>
