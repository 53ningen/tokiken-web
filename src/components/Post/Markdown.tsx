...
import { generateIndexId } from './PostBodyIndex'
      ...
      <ReactMarkdown
        components={{
          ...
          a: ({ children, href, ...props }) =>
            href ? (
              <Link href={href ?? '/'} target={href.startsWith('/') ? '_self' : '_blank'} {...props}>
                {children}
              </Link>
            ) : (
              <a {...props}>{children}</a>
          ...
          ul: ({ children, depth, ordered, ...props }) =>
            depth === 0 ? (
              <Box py={2}>
                <ul style={{ marginTop: 0, paddingTop: 0, paddingLeft: theme.spacing(2) }} {...props}>
                  {children}
                </ul>
              </Box>
            ) : (
          ...
          ol: ({ children, ordered, depth, ...props }) =>
            depth === 0 ? (
              <Box py={2}>
                <ol style={{ marginTop: 0, paddingTop: 0, paddingLeft: theme.spacing(2) }} {...props}>
                  {children}
                </ol>
              </Box>
            ) : (
